...
In order to support discoverability of Part 17 Exchange Set resources the
...
| Class | S100_SE_CertificateContainerType | A set of signed Public Key certificates | - | - | Used in S-100 Part 17 Exchange Catalogues
...
S100_SE_SignatureOnSignature element as described in <<cls-15-8.8>>. S-100 Part
17 metadata thus allows for multiple digital signatures, a single mandatory
...
| Enumeration
| S100_SE_DigitalSignatureReference
| A reference to a cryptographic algorithm used in an implementation of Part 15
|
